...
PB.start = function(url) {
    ...
    console.log('[SpectateUI] Connecting to ws backend on ' + this.backend);
    ...
    const connect = () => {
        ...
        this.socket.onopen = () => {
            ...
            console.log('[SpectateUI] Connection established!')
        };
        this.socket.onclose = () => {
            this.emit('statusChange', 'CLOSED');
            setTimeout(connect, 500);
            console.log('[SpectateUI] Attempt reconnect in 500ms');
        };
        this.socket.onerror = () => {
        ...
        this.socket.onmessage = msg => {
            ...
            if (data.eventType) {
                ...
                console.log('[SpectateUI] Unexpected packet format: ' + JSON.stringify(data));
            }
        };
    };
